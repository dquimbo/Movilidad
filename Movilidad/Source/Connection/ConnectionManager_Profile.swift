...
class ConnectionManager_Profile: ConnectionManager {
    ...
    func profile() -> Single<Profile?> {
        return Single.create { single in
            // Mock profile - no VPN needed
            let profile = XMLMapper<Profile>().map(XMLfile: "mock_profile.xml")
...
//            }
    ...
    func menu(profile: String) -> Single<Menu?> {
        return Single.create { single in
            // Mock menu - no VPN needed
            let menu = XMLMapper<Menu>().map(XMLfile: "mock_menu.xml")
            single(.success(menu))
            return Disposables.create { }
...
//            }
    ...
    func profileAdditionalInfo() -> Single<AdditionalProfileInfo?> {
        return Single.create { single in
            // Mock profileAdditionalInfo - no VPN needed
            let additionalInfo = XMLMapper<AdditionalProfileInfo>().map(XMLfile: "mock_additional_profile_info.xml")
...
//            }
    ...
    func notificationsCount() -> Single<Int> {
        return Single.create { single in
            // Mock notificationsCount - no VPN needed
            single(.success(0))
            return Disposables.create { }
...
//            }
    ...
    func registerPushNotificationToken(token: String) -> Single<Bool> {
        return Single.create { single in
            // Mock registerPushNotificationToken - no VPN needed
            single(.success(true))
            return Disposables.create { }
...
//            }
    ...
    func getProfilePhoto(username: String) -> Single<Data?> {
        return Single.create { single in
            // Mock getProfilePhoto - no VPN needed
            single(.success(nil))
            return Disposables.create { }
